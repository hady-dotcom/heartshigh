...
function buildClipCard(clip, kind) {
  ...
  top.appendChild(el('span', 'pill dur', isApp ? 'Extended cut' : '0:' + String(len).padStart(2, '0')));
  ...
  card.__replay = () => { if (card.__player) { try { card.__player.v.currentTime = clip.start; card.__player.v.play(); } catch (e) {} } };
...
function showEngagement(p, onShare, onSkip) {
  ...
  inr.appendChild(el('div', 'eyebrow', '❚❚ paused at ' + mmss(p.at) + ' · ' + esc(p.theme)));
  inr.appendChild(el('p', 'q-text', esc(p.prompt)));
  inr.appendChild(el('div', 'q-src', esc(p.promptSource)));

  if (p.quote) {
    const qq = el('div', '', '“' + esc(p.quote) + '”');
    qq.style.cssText = 'font-size:13px;line-height:1.55;color:var(--text);border-left:2.5px solid var(--purple);padding:2px 0 2px 11px;margin:0 0 14px';
    inr.appendChild(qq);
  }
  ...
  share.onclick = () => {
    shared = true;
    USER.answers[p.at] = { text: ta.value, private: priv };
    renderSwarm();
...
function growJibril() {
  ...
  J.branches.forEach(b => {
    ...
    c.appendChild(sl);
    t.onclick = () => c.classList.toggle('open');
...
function growGhuniyya() {
  ...
  ch.innerHTML = '<span class="pill hadith" style="background:rgba(122,79,163,.3);color:#D9C6EE">ḤADĪTH</span>' +
...
function growHarvest() {
  ...
  D.harvest.forEach(h => {
    ...
    top.innerHTML = '<span class="pill ' + (h.kind) + '">' + esc(h.tag) + '</span>' +
      '<span class="hc-ref">' + esc(h.ref) + (h.grading ? '<br><span style="font-weight:500">' + esc(h.grading) + '</span>' : '') + '</span>';
    c.appendChild(top);
    if (h.arabic) c.appendChild(el('div', 'ar', esc(h.arabic)));
    c.appendChild(el('div', 'hc-tr', '“' + esc(h.text) + '”'));
    if (h.metIn) {
      c.appendChild(el('div', 'hc-met', 'Met in: ' + esc(h.metIn) + '<br>' + esc(h.speaker) + ' · at ' + esc(h.ts)));
    } else {
      c.appendChild(el('div', 'hc-met', 'Met in the library' + (h.narrator ? ' · narrated by ' + esc(h.narrator) : '')));
    }
    const a = el('button', 'hc-act ' + (h.kind === 'quran' ? 'q' : 'h'), esc(h.cta));
    a.onclick = () => { if (h.videoId && VIDEO[h.videoId]) openMains(); else toast('Opens the source lecture'); };
    c.appendChild(a);
    s.appendChild(c);
  });
...
function growWorkbook() {
  ...
  entries.forEach(e => {
    const c = el('div', 'wb');
    c.appendChild(el('div', 'date', e.date + ' · ' + e.kind));
    c.appendChild(el('div', 'qlab', 'Video question was:'));
    c.appendChild(el('div', 'q', esc(e.q)));
    c.appendChild(el('div', 'a', '“' + esc(e.a) + '”'));
    ...
    if (e.private) {
      const p = el('span', 'pill', 'kept private');
      p.style.cssText = 'background:rgba(217,164,65,.18);color:#8A6A18;flex:0 0 auto';
      f.appendChild(p);
    }
    c.appendChild(f);
    s.appendChild(c);
